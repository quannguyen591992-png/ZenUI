...
- Guided Direction output contains only a bounded media plan (one Hero and at most three content intents with stable slot, `query` and localized `alt`). The model cannot submit a URL, Pexels result ID, object key or owned asset ID. The worker resolves one shared plan for all directions, tries bounded generated bytes first, then the fixed provider adapter and secure importer, and materializes only returned opaque ready asset IDs.
- Generated bytes cross the same hostile-image boundary as uploads/imports: bounded MIME/magic/pixels/bytes validation, metadata stripping, WebP normalization, private immutable object storage and integrity metadata. Prompt, enhanced prompt, raw bytes, model/provider metadata and source object keys are server-only and excluded from document/public DTO/queue/log surfaces.
- The hybrid image resolver is optional and fail-soft: generation safety/quota/auth/timeout or provider/storage/import failure cannot fail the three-direction run. Each explicit action gets at most one generated attempt and one Pexels fallback per slot; reload/resize/direction switch does not issue provider calls.
