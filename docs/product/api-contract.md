...
- `POST /api/v1/projects/:projectId/ai-proposals` accepts `workspaceId`, idempotent `requestId`, `action=request|refine|try-another`, optional `intent=standard|remix-section|replace-media`, `expectedVersion`, optional `selectedNodeId`, bounded prompt and replacement proposal ID where required. Scope and generation mode are derived server-side from the authorized current document; browser-authored scope, commands and proposal documents are rejected. Image-replacement language is deterministically routed to `replace-media` only when `selectedNodeId` names an exact authorized image or server-owned media slot; section/page targets fail closed.
...
- The worker compiles every validated route from the immutable snapshot, creates a sorted deterministic path-safe ZIP within the 10 MiB budget and uploads it to a private S3-compatible key. It never executes user code.
...
- The callback exchanges the one-time code only after atomic state consumption, validates exact configuration/team and minimum `deployment:read-write` plus `integration-configuration:read-write` scopes, then encrypts the access token with AES-256-GCM and tenant/connection/configuration-bound AAD.
...
- Deployment queue payloads contain local IDs only. The worker reads the immutable revision snapshot, compiles the bounded sorted static-site manifest, uploads one deterministic private bundle and sends all validated `{ path, content }` HTML files to Vercel. No user build command or generated JavaScript executes.
