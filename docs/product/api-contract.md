...
- A Guided Brief may leave `designSystem` absent or use `{ mode: "zenui" }`, preserving ZenUI's existing visual presets. `{ mode: "custom" }` requires exactly three structurally valid six-digit HEX colors, allowlisted heading/body fonts and bounded typography/spacing/radius presets. Low contrast produces a browser advisory rather than an API validation failure: the exact colors remain authoritative and direction generation may proceed. The browser preview and saving a brief never mutate a document, revision or autosave state.
- Custom Design System tokens are persisted in the existing private brief/run JSONB snapshots, but queue payloads remain local IDs only. The provider receives the content brief without `designSystem`; it cannot choose user visual tokens.
- Provider output is a strict `design-directions-v2` planner envelope: one shared content blueprint with one Hero media intent and exactly three unique feature-media intents (`feature-1`, `feature-2`, `feature-3`), plus exactly three direction entries containing only an allowlisted server-owned `presetId`. It cannot choose media keys, node IDs, raw style/theme values, HTML/CSS/JavaScript, SVG, arbitrary URLs, asset/provider IDs or mutation/publication actions.
- Duplicate allowlisted IDs are accepted only as planner proposals and repaired deterministically with recent-ID exclusion and structural-distance checks; unknown IDs and extra authority-bearing fields fail closed. No second provider call is used for repair. The server owns the final trio, versioned contracts and all-or-nothing materialization. In custom mode, every direction receives the same resolved colors/fonts/type scale/spacing/radius while retaining distinct bounded layout variants.
- Public run DTOs contain lifecycle, safe error and exactly three `{ id, name, character, rationale, document }` directions after completion. Provider/model/prompt/usage/content/visual-plan/exclusion/storage details are redacted.
...
- Guided creation resolves one shared Hero intent plus exactly three shared feature intents, using stable server-owned keys `shared-hero`, `shared-feature-1`, `shared-feature-2` and `shared-feature-3`. The worker requires the four-media product budget, batches resolutions at concurrency two, tries configured image generation once per intent, then fixed-provider Pexels import, and materializes the same successful ready opaque asset IDs into all three directions. A failed feature slot is omitted without an empty media panel; Hero retains its safe geometric fallback. Deterministic run+intent request IDs make retries idempotent.
