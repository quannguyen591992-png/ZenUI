...
6. Runtime rendering resolves canonical images as `${ASSET_ORIGIN}/a/<assetId>` for Canvas, isolated Preview and Share. Production requires an exact cookie-free asset hostname separate from the editor hostname. A public route returns only ready assets from private storage with exact `image/webp`, content length, strong ETag, `Cache-Control: public, max-age=31536000, immutable`, `X-Content-Type-Options: nosniff`, `Cross-Origin-Resource-Policy: cross-origin`, and no cookie or tenant/object/provider metadata.
7. Export and Deploy are portable immutable artifacts: the worker authorizes every referenced project/workspace asset against the selected project, reads its private object, verifies exact byte length and SHA-256, compiles depth-correct relative `assets/<assetId>.webp` URLs with CSP `img-src 'self'`, and packages the exact WebP bytes. Artifacts never contain `ASSET_ORIGIN`, object keys or storage credentials. Legacy allowlisted remote URLs remain read-compatible and retain their exact CSP sources.
8. Public asset URLs are unguessable bearer references, not an authorization boundary for confidential media. Random UUIDs prevent practical enumeration; asset IDs are not derived only from public content hashes. Deleting, revoking or private-media access semantics are outside this generated public landing-page stage.
...
- Generated production pages no longer contact Pexels/Unsplash. Runtime surfaces share one immutable cookie-free asset URL, while Export/Deploy package the same verified bytes behind relative same-site paths.
