...
test('prepares exactly three directions with one provider call and accepts one into the editor', async ({ page }) => {
  ...
  await expect(preview.locator('#features-section')).toHaveCount(1)
  await expect(preview.locator('#lead-form-1')).toHaveCount(1)
  await preview.locator('a[href="#features-section"]').first().click()
  await expect.poll(() => preview.locator('html').evaluate(() => location.hash))
    .toBe('#features-section')
  await expect(preview.locator('#features-section')).toBeInViewport()
  await preview.locator('a[href="#lead-form-1"]').first().press('Enter')
  await expect.poll(() => preview.locator('html').evaluate(() => location.hash))
    .toBe('#lead-form-1')
  await expect(preview.locator('#lead-form-1')).toBeInViewport()
  ...
  expect(sharedHtml).toContain('href="#features-section"')
  expect(sharedHtml).toContain('id="features-section"')
  expect(sharedHtml).toContain('href="#lead-form-1"')
  expect(sharedHtml).toContain('id="lead-form-1"')
  expect(sharedHtml).not.toContain('#start')
  await page.getByRole('button', { name: 'Chia sẻ', exact: true }).click()
