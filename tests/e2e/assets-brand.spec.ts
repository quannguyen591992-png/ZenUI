...
test('imports a fixed-provider result and applies a versioned Brand Kit', async ({ page }) => {
  ...
  const importResult = page.getByRole('button', { name: 'Nhập ảnh Bảng lập kế hoạch ra mắt' })
  await expect(importResult).toBeVisible({ timeout: 15_000 })
  await importResult.click()
