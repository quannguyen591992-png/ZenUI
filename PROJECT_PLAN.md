...
| D-030 | 2026-08-19 | AI usage dùng accounting đa phương thức bất biến: thu mọi Gemini response có image bytes tại provider boundary, snapshot giá text/image riêng, Pexels là stock và không backfill đoán lịch sử | Accepted; implemented | Chi phí provider phát sinh trước import/judge/selection và không thể suy ra đúng từ text token, ảnh được chọn hoặc cấu hình hiện tại | Migration 0019 nullable; Dashboard cộng known text/image cost và giữ unknown là partial/unpriced; PostgreSQL là authority, LangSmith metadata-only; xem ADR-0021 |
...
- **Follow-up Lead Form Inspector Premium + bảng Thiết kế sticky đã hoàn tất implementation và automated verification ngày 2026-08-14.** Lead Form builder dùng card/control/action hierarchy của Premium Editor; desktop Simple/Advanced Inspector bám viewport, cuộn độc lập và giữ thanh lưu ở đáy; mobile tiếp tục dùng edit sheet cuộn bình thường. Không đổi Lead Form schema, command/history/autosave, renderer/publication, Customer Leads, AI, auth hoặc `.env`.
...
- [x] Follow-up Lead Form Inspector Premium + sticky Design panel ngày 2026-08-14: TDD RED đúng accessible structure còn thiếu (`Nội dung biểu mẫu`); production giữ nguyên command/Zod behavior nhưng tổ chức builder thành card bố cục, nội dung, trường, consent và action, dùng `inspector-field-group`/`pro-input`, button hierarchy primary/secondary/tertiary/danger, focus/disabled/touch state. Desktop `.section-guide` và `.inspector-panel` sticky, viewport-bounded, `overflow-y:auto`, overscroll contained; project context chừa 52px tabs và toolbar; `.lead-form-save-bar` sticky desktop, static trong mobile sheet. GREEN focused Vitest editor 76/76 (target kéo theo ba files) và Simple 33/33; focused Lead Form E2E Chromium/Firefox/WebKit lần lượt 1/1, gồm desktop geometry, independent scroll, no horizontal overflow, mobile 390×844 sheet và axe serious/critical 0. Lượt WebKit đầu bị E2E reset 404 khi chạy song song hai Playwright web-server stacks; rerun cô lập pass. Full Web 44 files/370 tests; Web coverage statements 84.89%, branches 80.06%, functions 82.18%, lines 88.74%; workspace typecheck/lint/build 17/17; `git diff --check` không có whitespace error, chỉ LF→CRLF notices. Không gọi provider live/trả phí, không sửa `.env`, login, schema, public renderer hoặc Customer Leads; giữ nguyên thay đổi ngoài scope `note.txt` và `apps/web/next-env.d.ts`.
- [x] Follow-up control Khoảng cách giữa các phần tử ngày 2026-08-14: root cause là Inspector cũ luôn ghi `style.gap` cho mọi node dù CSS gap chỉ tạo thay đổi nhìn thấy trên layout flex/grid có nhiều phần tử con; container block và leaf node vì vậy lưu command thành công nhưng Canvas không đổi. TDD RED xác nhận accessible name/hành vi mới chưa tồn tại; fix chỉ hiển thị `Khoảng cách giữa các phần tử` cho container flex/grid có ít nhất hai child, cùng ngoại lệ Lead Form dùng semantic grid và synthetic fields. Slider tiếp tục cập nhật Canvas trực tiếp qua `UPDATE_STYLE` trên desktop hoặc `UPDATE_RESPONSIVE_STYLE` trên tablet/mobile; numeric blur validation, undo/redo, autosave, reload và giới hạn 0..200 giữ nguyên. Không đổi gap thành padding/margin và không sửa schema/compiler/renderer. GREEN focused editor 1/1 rồi full file 38/38; full Web 44 files/371 tests; Playwright geometry thật 1/1 trên Chromium, Firefox và WebKit xác nhận khoảng cách 16px→48px, persistence, undo/redo và control không xuất hiện trên layout không hỗ trợ. Web coverage statements 84.89%, branches 80.02%, functions 82.10%, lines 88.74%; workspace typecheck/lint/build 17/17; `git diff --check` pass ngoài LF→CRLF notices. Không gọi provider live/trả phí, không sửa `.env`, auth, AI, publication hoặc Customer Leads; giữ nguyên thay đổi ngoài scope `note.txt` và `apps/web/next-env.d.ts`.
- [x] Follow-up application shell và quyền sở hữu scroll editor ngày 2026-08-14, supersede phần sticky panel của mục 2710: TDD RED xác nhận Canvas chưa có vùng cuộn chung với Ảnh/thương hiệu và textarea `Nội dung` chưa tự giãn. Desktop editor nay bị chặn đúng phần viewport dưới tab dự án 52px; toolbar/status ở hàng cố định, body/document không cuộn, chỉ `.editor-center-scroll` cuộn Canvas + Ảnh/thương hiệu. Câu chuyện trang và Thiết kế mỗi cột có tối đa một scroll owner khi nội dung dài; bỏ sticky cho chính panel nhưng giữ Lead Form save bar sticky bên trong cột Thiết kế. Textarea Nội dung controlled tự giãn theo `scrollHeight` tới 240px rồi mới dùng scrollbar riêng. Page Manager, Simple, Advanced và mobile sheet giữ nguyên command/history/autosave/focus behavior; mobile 390×844 không overflow ngang và axe serious/critical 0 trong các journey bị ảnh hưởng. GREEN Web 44 files/373 tests; workspace coverage 31/31 tasks, Web 84.93% statements, 80.01% branches, 82.11% functions, 88.77% lines; typecheck/lint/build 17/17. Affected Playwright: Chromium 5/5 qua hai lượt cô lập, Firefox 5/5, WebKit 4/5 ở lượt đầu rồi Lead Form retry 1/1; failure WebKit đầu là reload persistence không tái hiện. Một lượt Chromium đồng thời trả `accept-starter` 404 cho cả 5 test, retry cô lập pass; full Web đầu timeout riêng `export-panel`, retry file 4/4 và full Web pass; coverage đầu làm PGlite process thoát khi chạy đồng thời, database isolation 87 tests đạt 87.59/80.16/96.96/94.26 rồi full workspace rerun pass. `git diff --check` không có whitespace error, chỉ LF→CRLF notices. Không gọi provider live/trả phí, không sửa `.env`, auth, Design Document/schema/compiler/publication hoặc Customer Leads; giữ nguyên `note.txt` ngoài scope.
- [x] Follow-up semantic navigation + CTA Đăng ký ngày 2026-08-14: navbar do AI/provider sinh nay chỉ mang semantic target thuộc allowlist `logo-cloud|stats|features|testimonials|pricing|faq|final-cta`; target trùng, giả mạo hoặc không có trong section set bị Zod/semantic validation từ chối. ZenUI server ánh xạ target thành canonical node ID, mọi compiler/React node có `id` thật, Lead Form CTA giữ action server-owned `lead_form → lead-form-1`, còn conversion không có form chỉ trỏ `final-cta-section`; generated Blueprint v2 không còn `#start` hoặc fragment treo. Compiler dùng native fragment + `scroll-behavior:smooth`, `scroll-margin-top:96px` và reduced-motion fallback, không thêm script; CSP vẫn `script-src 'none'`, `connect-src 'none'`. Secure Preview có interaction mode tường minh: user-facing Xem trước dùng `presentation` và cho browser điều hướng native, runtime `inspect` vẫn preventDefault + gửi `NODE_CLICKED`, submit form Preview luôn bị chặn. Deterministic Guided fixture chỉ tạo navigation đến section thật có trong brief. RED evidence: Blueprint 13 fail/2 pass, compiler 1 fail/21 pass, Preview 1 fail/6 pass, renderer thiếu Lead Form ID, SecurePreview 1 fail/5 pass; GREEN focused AI 39, compiler 22, Preview 7, renderer/SecurePreview 10, Worker 47 và database direction 9 tests. Full workspace `pnpm test` pass 31/31 tasks (Web 374, AI Core 101, database 87, Worker 81); coverage rerun error-scoped pass 31/31 với mọi configured threshold >=80%; typecheck/lint/build 17/17. E2E Share Chromium 3/3; Guided 12/12 trên Chromium/Firefox/WebKit, gồm mouse nav, keyboard Đăng ký, desktop/mobile và axe; presentation Preview regression Chromium 1/1. Một lượt Guided cross-browser đầu trả E2E reset 404 cho 12/12 do transient E2E server/runtime startup, isolated Chromium và clean full rerun đều pass. `git diff --check` không có whitespace error, chỉ LF→CRLF notices; scan generated AI Core không còn `#start`, compiler CSP giữ nguyên và secret scan chỉ thấy tên env/test placeholder có sẵn. Stored Design Direction v1 không cần read migration cho hành vi này: UI/API trả materialized `directionSnapshots` (Design Document) và Choose dùng snapshot; `contentBlueprint` lịch sử không được parse trên read/accept path, trong khi output mới/complete vẫn fail closed theo v2. Không gọi Gemini/Pexels/Vercel provider live, không sửa `.env`/auth, không commit/push và giữ nguyên `note.txt` cùng `apps/web/next-env.d.ts` ngoài scope.
- [x] Follow-up public Asset Origin + local bind origin ngày 2026-08-17: owner mở managed Share qua HTTPS ngrok và thấy layout/text đầy đủ nhưng owned image chỉ hiện alt text. Direct inspection xác nhận Share HTML phát `src=http://127.0.0.1:3002/a/<UUID>` trong khi asset local vẫn 200 `image/webp`, nên nguyên nhân là loopback/mixed-content chứ không phải mất asset hay regression navigation. Runtime nay tách public `ASSET_ORIGIN` khỏi optional local-only `ASSET_SERVER_ORIGIN`; biến mới chỉ nhận root loopback HTTP origin, được dùng cho listener, port preflight và health readiness, fallback về `ASSET_ORIGIN` để cấu hình cũ chạy nguyên trạng. Asset proxy tiếp tục gọi Web nội bộ nhưng gửi exact public Host, strip Cookie/Set-Cookie và chỉ nhận GET `/a/<UUID>`; compiler/Share vẫn dùng exact public origin trong URL/CSP và không nới `script-src 'none'`/`connect-src 'none'`. `.env.example` giữ mặc định tương thích; local-live runbook dùng topology một tunnel `ngrok http 3000`, trong đó exact public hostname phục vụ cả `/s/<slug>` và `/a/<asset-id>`, còn Editor giữ `APP_ORIGIN=http://localhost:3000` và Asset Server chỉ bind `ASSET_SERVER_ORIGIN=http://127.0.0.1:3002`. Theo yêu cầu trực tiếp của owner, `.env` thật chỉ được cập nhật bốn origin theo topology này, không đụng secret/biến khác. TDD RED compile-time đúng vì thiếu export `resolveAssetServerOrigin`; GREEN `pnpm dev:test` 8/8. Focused Web public asset/Share 22/22 và compiler 22/22; full workspace test 31/31 tasks (Web 374, database 87, Worker 81, AI Core 101); typecheck/lint/build 17/17. Coverage lượt đầu gặp một PGlite `beforeEach` timeout 10 giây ở generation repository trong khi 86/87 database tests pass; isolated file sau đó 16/16 và clean full coverage rerun pass 31/31 với mọi configured threshold >=80%, xác nhận contention hạ tầng không tái hiện. Clean restart báo Web/Preview/Asset/Worker ready và cả bốn local endpoint 200. Tunnel smoke xác nhận root 200, direct known `/a/<UUID>` 200 `image/webp` 96.478 byte; link owner đưa đã bị disabled lúc 2026-08-17 01:57:52 UTC nên 404 đúng contract. Một active immutable Share trả 200, phát đủ 4 image URL cùng public ngrok origin; cả 4 asset trả 200 `image/webp` (327.140 / 92.250 / 103.638 / 89.242 byte), CSP `img-src` chứa exact ngrok origin và vẫn giữ `script-src 'none'`/`connect-src 'none'`. Không gọi provider live. `apps/web/next-env.d.ts` do build sinh đã được restore, `note.txt` giữ ngoài scope; chưa commit/push task này.
- [x] Follow-up Landing Page Visual Polish — Bước 1 ngày 2026-08-17: thêm optional strict `theme.presentation` v1 với profile allowlist `refined|dynamic|editorial` và language `vi|en`; document cũ fallback `refined`. `styleSchema.shadow` mở rộng bounded `xs|sm|md|lg|xl`, trong đó `xl` map cố định `0 32px 80px rgba(15,23,42,.22)`. Shared presentation helper trong HTML compiler sở hữu toàn bộ selector/keyframe và được dùng chung bởi React Canvas, isolated Preview, Managed Share/Export/Deploy compiler; root mang `data-zenui-render-root` + `data-visual-profile`. CSS được scope dưới render root, hover chỉ chạy trong `@media(hover:hover) and (pointer:fine)`, có focus-visible, button active, link underline, feature-card lift, image micro-zoom, form-control transition và reduced-motion tắt toàn bộ animation/transition/transform không thiết yếu. Chỉ profile `dynamic` phát keyframe allowlisted `zenui-dynamic-float`; schema strict từ chối raw CSS, JavaScript, selector, keyframe và unknown profile. CSP vẫn `script-src 'none'`, `connect-src 'none'`; không thêm listener/script hoặc quyền style cho AI/provider. RED hợp lệ: Design Schema 1/55 fail do thiếu `xs`; compiler 1/23 fail do presentation contract; Preview 1/8 và React renderer 1/5 fail do thiếu root/profile marker. GREEN focused: Design Schema 55/55, HTML Compiler 23/23, Preview 8/8, React renderer 5/5; typecheck cả bốn package pass. Coverage: Design Schema 95.47/93/95.74/97.52, Compiler 93.33/87.64/100/96.90, Preview 98.83/82.97/100/98.63 và full Web 44 files/375 tests với 84.93/80.01/82.11/88.77 (statements/branches/functions/lines), đều >=80%. Focused Web-only coverage 5/5 có branches 76.31% nên exit 1; full configured Web gate ngay sau đó pass và supersede kết quả subset. `git diff --check` không có whitespace error, chỉ LF→CRLF notices. Bước 2 font self-hosted chưa bắt đầu; không gọi provider live, không sửa `.env`/auth, không commit/push và giữ `note.txt` ngoài scope.
- [x] Follow-up Vietnamese Font Library — Bước 2 ngày 2026-08-17: thêm package browser-safe/Node-only `@zenui/font-library` với năm family OFL allowlisted (`Manrope`, `Be Vietnam Pro`, `Inter`, `Noto Sans`, `Noto Serif`), mỗi family chỉ có static 400-normal Latin + Vietnamese WOFF2 đã commit kèm license/provenance, byte size và SHA-256 manifest; mỗi document tối đa hai hosted family và system font không tạo network reference. Design Schema mở allowlist nhưng vẫn từ chối arbitrary font URL, route segment `f` được reserve. Public route exact-host `/f/<font-id>/<latin|vietnamese>.woff2` validate Zod/path/checksum/size, trả fixed `font/woff2`, immutable cache, CORS `*`, CORP cross-origin, `nosniff`, không cookie và uniform fail-closed; local port-3000 proxy chỉ nhận exact GET path, strip Cookie/Set-Cookie và gửi public ASSET_ORIGIN Host. Canvas, Preview và compiler dùng cùng catalog-generated `@font-face`; Preview CSP chỉ cho exact asset origin, Managed Share chỉ phát exact `font-src` khi cần. Export/Deploy bundle đúng hai subset của family đang dùng dưới `fonts/`, dùng relative path cho nested routes, `font-src 'self'`, tính font bytes + file count vào giới hạn artifact và từ chối binary rỗng/quá 128 KiB/checksum sai. RED hợp lệ: compiler thiếu font-face/lang/CSP portable; public route/proxy chưa tồn tại; Worker `loadFont` 0 lần và chưa chặn subset quá giới hạn; React Canvas không có hosted font stylesheet. GREEN: Font Library 5/5; HTML Compiler 25/25; Preview 9/9; local runtime proxy 9/9; Web font/renderer focused 8/8; Worker full 8 files/83 tests. Coverage (statements/branches/functions/lines): Font Library 96.96/90.90/100/96.87; Compiler 93.28/87.96/100/97; Preview 98.85/82.97/100/98.64; Worker 86.21/80.47/86.40/89.36; full Web 45 files/378 tests đạt 84.94/80/82.19/88.79. Font Library/Compiler/Preview/Web/Worker typecheck và lint đều pass; production build của cả năm target pass. Lượt Web Turbopack đầu chỉ báo một lỗi bị ẩn; Webpack diagnostic xác định Node entry import `./index.js` không resolve khi workspace export source TypeScript, đã đổi sang self-reference package export `@zenui/font-library`, rerun focused test/typecheck/lint và production builds đều pass. Security regression giữ publication `script-src 'none'`, `connect-src 'none'`, exact/portable `font-src`; scan không phát hiện Google Fonts/CDN hoặc quyền font/CSS/JavaScript tùy ý trong production path. `git diff --check` không có whitespace error, chỉ LF→CRLF notices. Không gọi provider live, không sửa `.env`/auth, không commit/push và giữ `note.txt` ngoài scope.
- [x] Follow-up Landing Page Advanced Effects — Bước 3 và final gate ngày 2026-08-17: `refined`, `dynamic`, `editorial` dùng profile CSS-only do ZenUI sở hữu; refined reveal 700ms/4px, dynamic reveal 760ms/6px với stagger cố định, static hero glow và đúng một ambient hero-image float 6s, editorial reveal 860ms/5px kèm divider/typographic emphasis. Pointer fine mới nhận hover; touch/coarse pointer và `prefers-reduced-motion` neutralize animation/transition/transform. Server materializer map mood `editorial→editorial`, `bold→dynamic`, còn lại `refined`; custom typography `expressive→dynamic`, còn lại `refined`; Design Directions dùng validated content language. Provider schema tiếp tục không có presentation/profile/effect/selector/keyframe/rawCss/javascript authority. TDD RED hợp lệ: Compiler 1/26 fail vì thiếu refined keyframe; bảy Blueprint mapping case fail vì `theme.presentation` chưa materialize. GREEN focused Compiler 26/26, Blueprint + Design Direction 40/40; full AI Core 108/108. Final workspace: tests 33/33 tasks (Web 45 files/378 tests, Worker 83 tests), coverage 33/33 với mọi configured threshold >=80% — Font Library 96.96/90.90/100/96.87, Compiler 93.30/88.10/100/97.02, AI Core 90.38/81.92/96.92/94.28, Preview 98.85/82.97/100/98.64, Web 84.94/80/82.19/88.79, Worker 86.21/80.47/86.40/89.36; typecheck/lint/build đều 18/18 tasks. Runtime gate phát hiện hai defect chỉ lộ khi chạy thật: Preview Node strip-types không resolve import `.js`, đã dùng explicit `.ts` cùng `allowImportingTsExtensions`; Turbopack biến dynamic font asset URL thành một static file đầu tiên, đã thay bằng exact lazy static loader matrix cho 10 subset; local proxy bổ sung canonical `x-forwarded-proto` để exact HTTPS public host qua HTTP loopback không bị 404. Web production build sau lazy-loader refactor pass và không còn cảnh báo whole-project NFT tracing; route manifest chỉ trace 10 emitted WOFF2 cùng runtime dependencies, không còn source test/coverage/project files. Regression GREEN: Preview typecheck/lint/test 9/9/build; Font Library 5/5; public font 3/3; dev runtime 9/9. Clean topology Web/Preview/Asset/Worker đều HTTP 200; local proxy và public ngrok `/f/noto-serif/vietnamese.woff2` đều trả 200 `font/woff2`, 5.240 byte, signature `wOF2`, immutable cache, CORS `*`, CORP cross-origin và không Set-Cookie; Preview CSP giữ `connect-src 'none'` với exact public `font-src`. Playwright Chromium critical Preview/Export/Share 5/5 pass, gồm isolated Preview, multi-route ZIP, immutable Share, managed lead và outsider/forged-origin guard. Publication vẫn giữ `script-src 'none'`, `connect-src 'none'`; không thêm JavaScript tùy ý, không gọi provider live, không sửa `.env`/local auth/GitHub OAuth, không commit/push và giữ `note.txt` ngoài scope.
- [x] Follow-up Guided Design Direction strict-output normalization ngày 2026-08-17: owner tạo project mới và run `6e0a66c4-80ab-4bf2-81c1-09813af6d273` fail `invalid_model_output` sau một Gemini response có usage 919/1481/2400 token; không có asset nào được tạo nên lỗi xảy ra tại structured plan validation trước media resolution. Root cause là provider schema không thể biểu diễn các quan hệ chéo giữa `navigation.target`, `sectionOrder`, required brief sections và Footer-last; một output vẫn đúng kiểu/allowlist nhưng thiếu section được navigation trỏ tới bị strict semantic `superRefine` từ chối toàn bộ. Fix giữ raw object/unknown key/enum/media/cardinality fail-closed, nhưng tách structural base schema khỏi semantic schema và server-normalize duy nhất quan hệ section: dedupe section order, thêm required brief/navigation target, đặt Footer cuối, rồi parse lại bằng strict semantic schema trước preset resolution/materialization. AI/provider vẫn không có visual/CSS/JavaScript/node/asset authority và không có provider retry/call thứ hai. TDD RED regression nhận `accepted=false`; GREEN focused Design Directions 19/19, full AI Core 8 files/109 tests, AI Core typecheck/lint và scoped diff check pass. Clean restart xác nhận Web/Preview/Asset/Worker HTTP 200, Worker instance mới `7fc289c9bcde81c8b478b518ef1e4959` với `generation,asset,export,deployment`. Không tự gọi lại Gemini; owner sẽ bấm `Điều chỉnh bản mô tả` rồi tạo lại đúng một lần để kiểm thử thực tế. Không sửa `.env`/auth, không commit/push và giữ `note.txt` ngoài scope.
- [x] Follow-up Guided navigation materialization ngày 2026-08-17: owner retry cùng project với brief chỉ chọn `introduction`, `benefits`, `contact`; hai run `454ec363-12ab-45d5-821a-9fc43cad8df2` và `24b78596-7beb-4b83-a639-9f7bbb409d17` đều fail `invalid_model_output` sau response 919/1480/2399 và 919/1475/2394 token. Cả hai lượt đã tạo đủ bốn owned generated assets ở trạng thái `ready`, nên structural parse, section normalization và media resolution đều đã qua; lỗi nằm tại materialization cuối. Root cause là normalization đã thêm allowlisted `navigation.target` vào `sectionOrder`, nhưng `sectionsFor` sau đó chỉ giữ required brief sections + logo/stats, làm mất target provider hợp lệ không được owner đánh dấu bắt buộc; canonical navbar href vì vậy trỏ tới node không tồn tại và strict Design Document materializer từ chối toàn bộ. Fix tối thiểu lấy hợp của required brief sections và validated navigation targets khi materialize; không đổi schema/provider/media budget, không nới CSS/JavaScript/node/asset authority và không gọi provider lần hai. TDD RED hợp lệ: test mới compile/chạy, 1 fail/19 pass với `accepted=false`; GREEN focused Design Directions 20/20, full AI Core 8 files/110 tests, package + workspace typecheck 18/18, scoped + workspace lint 18/18 và full `git diff --check` pass ngoài LF→CRLF notices. Runtime watch đã restart Worker khi `design-directions.ts` đổi; Web/Preview/Asset/Worker đều HTTP 200, instance `7fc289c9bcde81c8b478b518ef1e4959` có `generation,asset,export,deployment`. Không tự gọi Gemini/Image/Pexels, không sửa `.env`/auth, không commit/push và giữ `note.txt` ngoài scope; chờ owner retry thực tế đúng một lần.
- [x] Follow-up LangSmith AI Observability metadata-only ngày 2026-08-17: Worker có adapter OpenTelemetry OTLP/HTTP riêng, disabled mặc định, không thêm LangChain/auto-instrumentation và không thay Prometheus/Grafana hoặc PostgreSQL `usage_records`. Root chain bao phủ Generation/Proposal/Design Direction; Gemini text/planner/judge/image dùng child LLM spans trong active root. Contract chỉ nhận enum/count/provider/model/prompt version/safe outcome-error/token totals cùng HMAC-SHA-256 correlation digest; không có generic attribute API, không `recordException`, không gửi prompt/brief/Design Document/generated content/provider payload/Lead data/PII/resource ID/private URL/object key/credential/error message hoặc stack trace. Config khi enabled fail startup với missing/placeholder key, weak correlation secret, HTTP/credentialed/query endpoint và numeric bounds sai; exporter/flush/shutdown sau startup fail-open, bounded. TDD RED hợp lệ: focused command fail vì module `../src/ai-observability.js` chưa tồn tại; GREEN focused 3 files/72 tests và full Worker 9 files/93 tests. Worker coverage đạt statements 86.60%, branches 81.02%, functions 88.34%, lines 89.59%; workspace test serial pass 33/33 tasks (Web 379, Worker 93), typecheck/lint/build pass 18/18 tasks. `git diff --check` không có whitespace error ngoài LF→CRLF notices; bounded privacy scan không tìm `recordException`, auto-instrumentation hay capture input/output; credential scan chỉ thấy giá trị test `lsv2_test_private_key` và placeholder tài liệu. Diff không chạm local auth/GitHub OAuth hoặc `.env` thật; file Next generated sau build đã restore, `note.txt` giữ ngoài scope. Disabled-mode clean runtime smoke khởi động Web/Preview/Asset/Worker, landing page có ZenUI và các health endpoint trả 200; local `.env` không đặt `LANGSMITH_TRACING_ENABLED`, nên không có OTLP/provider call. Không chạy live LangSmith/Gemini/Pexels/Vercel, không dùng credential thật và không commit/push.
- [x] Follow-up Dashboard Khách hàng + Sử dụng AI ngày 2026-08-18: shared Dashboard shell có route thật `Dự án`, role-aware `Khách hàng`, `Sử dụng AI`, giữ `Mẫu/Cài đặt` ở trạng thái Soon và loại bỏ `Tài nguyên/Nhóm`; local logout và GitHub OAuth khóa giữ nguyên. Customer Leads chuyển thành workspace-wide redacted inbox, lọc project/status, phân trang, viewer bị chặn và detail/PATCH tiếp tục project-scoped, authorize-before-decrypt, exact-Origin + optimistic version; Editor chỉ giữ shortcut/badge. `@zenui/usage-core` sở hữu strict query/report contract, exact-match pricing catalog `google-gemini-2026-08-13`, integer micro-USD half-up và timezone helper; migration 0018 snapshot giá nullable bất biến vào `usage_records`. Generation/Design Direction success/failure/stale đều persist token/price idempotently; unknown/legacy record là `unpriced`, không tính zero hoặc backfill đoán giá. Usage repository/API hard-bind current authenticated user + workspace kể cả owner, bounded 1–90 ngày, filter/pagination/server daily series; response không chứa prompt/brief/document/provider payload/PII/LangSmith ID. UI có KPI, SVG chart + screen-reader table, filters, table, loading/error/retry/empty/unpriced warning và responsive local table scroll; LangSmith vẫn chỉ observability, không phải accounting. TDD RED/GREEN focused: Usage UI + API 2 files/7 tests; route-dependency regression 3/3; Usage Core 10/10. E2E-only seed route guarded bởi `ZENUI_E2E_ENABLED`, không gọi provider live; Next giữ PGlite server-external và E2E database state dùng `globalThis` để các route/layout module chunks chia sẻ một instance đã migrate. Final focused E2E `usage-dashboard.spec.ts` pass 2/2 trên Chromium, 2/2 Firefox và 2/2 WebKit; Firefox `NS_BINDING_ABORTED` khi gọi `page.goto` từ trang Dashboard được sửa bằng hành vi người dùng click link `Sử dụng AI`, sau đó cả ba browser xanh. Documentation ghi rõ estimated USD, current-user-only, immutable catalog snapshot và unpriced subtotal. Final workspace gates: `pnpm test` pass 35/35 tasks (Web 47 files/394 tests); `pnpm test:coverage` pass 35/35 tasks, Web 48 files/397 tests đạt statements 85.25%, branches 80.04%, functions 82.52%, lines 89.10%, Usage Core đạt 100%/96.77%/100%/100%; `pnpm typecheck`, `pnpm lint` và `pnpm build` đều pass 19/19 tasks. Production build có warning Windows non-fatal về link name dài của PGlite nhưng hoàn tất đủ Web/Worker/Preview. `git diff --check` không có whitespace error, chỉ LF→CRLF notices. Bounded security review xác nhận workspace/current-user hard-binding, project ownership filter, authorize-before-decrypt, exact-Origin PATCH, E2E route fail-closed, immutable pricing snapshot và không lộ prompt/PII; `.env` thật, local auth, GitHub OAuth khóa, quota/admission/token budget không bị sửa. Không gọi Gemini/LangSmith/Pexels/Vercel và chưa commit/push; `note.txt` tiếp tục giữ ngoài scope.
- [x] Follow-up local AI Usage schema migration ngày 2026-08-18: owner mở `/dashboard/usage` nhận safe error vì API trả 500. Runtime log xác nhận request hợp lệ dùng `timezone=Asia/Saigon`; live PostgreSQL inspection cho thấy `usage_records` mới chỉ có schema trước Dashboard, thiếu toàn bộ pricing snapshot columns của migration 0018, nên repository select theo schema mới thất bại. Đây là deployment/migration gap, không phải lỗi timezone, authentication hoặc dữ liệu usage. Lượt `pnpm db:migrate` đầu fail đúng vì shell không nạp `.env`; rerun với `DATABASE_URL` local chỉ trong process environment áp dụng migration thành công, không sửa `.env` và không xóa/reset volume hay dữ liệu. Schema live sau migration có đủ bảy pricing columns; authenticated request cùng workspace, 30 ngày và `Asia/Saigon` trả 200 với 150 records, token totals/series/pagination đầy đủ. Record lịch sử vẫn hiển thị `unpriced` đúng contract vì không backfill đoán giá. Web/Preview/Asset/Worker và PostgreSQL/Redis/MinIO tiếp tục ready; không gọi provider live, không sửa auth hoặc code production.
- [x] Follow-up pricing `gemini-3.1-flash-lite` ngày 2026-08-18: owner yêu cầu bổ sung exact model đang dùng sau khi Dashboard hiển thị 150 record lịch sử `Chưa có giá`. Nguồn chính thức Gemini API Pricing, cập nhật 2026-08-13 UTC, xác nhận Standard paid tier cho text/image/video input USD 0.25/1M token và output USD 1.50/1M token (đã gồm thinking tokens); audio/context caching có mức riêng nhưng usage contract hiện không phân loại modality/cache nên không được áp dụng lẫn. TDD RED hợp lệ: `resolveUsagePricing()` trả `null` thay vì exact rate; GREEN thêm `google-gemini` + `gemini-3.1-flash-lite` vào catalog version `google-gemini-2026-08-13`, snapshot 1M input + 0.5M output thành 1,000,000 micro-USD. Focused Usage Core 11/11, typecheck và lint pass; run mới sau hot reload sẽ lưu immutable priced snapshot. 150 record lịch sử tiếp tục `unpriced`, không backfill đoán giá. Không gọi provider live, không sửa `.env`, auth, quota/admission/token budget hoặc generation behavior.
- [x] Follow-up AI Usage token/media cost semantics ngày 2026-08-18: owner xác nhận Design Direction mới dùng `gemini-3.1-flash-lite` ghi 920 input + 1,529 output = 2,449 token và snapshot `2524` micro-USD; phép tính đúng theo rate USD 0.25/1M input + USD 1.50/1M output, nhưng bảng chi tiết hiển thị đơn vị micro-USD khó đọc. UI nay dùng cùng formatter USD với KPI, nên `2524` micro-USD hiển thị `$0.002524`. Đây là subtotal token đã xác định, không phải full provider cost: Design Direction có thể tạo/resolve media nên record được trả với status `partial`, giữ `$0.002524` và hiện `Giá ảnh chưa bao gồm`; cảnh báo tổng hợp nói rõ các lượt có chi phí chưa đầy đủ và tổng chỉ gồm phần đã xác định. Live durable metadata của proposal thay ảnh `24bc732d-86e4-49f0-a05a-dc549e9430bd` xác nhận `proposal_intent=replace-media`, token text 0/0/0 và snapshot cũ 0; replace-media có image generation hoặc stock resolution nhưng schema accounting hiện chỉ có token pricing, nên zero token không chứng minh zero provider cost. TDD RED hợp lệ: Usage Core trả priced zero cho unsupported provider operation; Usage UI chỉ có một `$0.000053` vì row còn `53 micro-USD`; database historical-media regression nhận `priced` thay vì `unsupported_media_cost`; test cảnh báo UI vẫn nhận `lượt gọi chưa có giá` thay vì contract chi phí chưa đầy đủ. GREEN: `createUsagePricingSnapshot` phân biệt `priced|partial|unpriced` với reason `unknown_model|unsupported_media_cost`; Design Direction persist known token subtotal dưới dạng `partial`; generation persistence không snapshot text price cho replace-media mới; report left-join generation intent để record lịch sử replace-media zero-token bị loại khỏi priced subtotal và hiện `Chưa có giá ảnh`, trong khi historical Design Direction có token snapshot được phân loại `partial`; không xóa/backfill dữ liệu. Focused GREEN: Usage Core 12/12; database generation 17/17, design-direction 11/11 và usage 3/3; Web Usage UI/API 7/7. Usage Core coverage 100% statements, 97.67% branches, 100% functions/lines; typecheck/lint Usage Core, Database, Web và production build ba target pass; sau wording cuối, Web typecheck/lint cùng Usage UI/API 7/7 tiếp tục pass. `git diff --check` không có whitespace error ngoài LF→CRLF notices; Web readiness trả 200 với PostgreSQL/Redis ready. Không gọi provider live, không đổi generation/quota/admission/token budget, không sửa `.env`/auth và không commit/push.
- [x] Follow-up immutable multimodal AI Usage accounting ngày 2026-08-19: owner yêu cầu model/chi phí ảnh thật thay vì tiếp tục hiển thị 0 hoặc thiếu giá. `@zenui/usage-core` thêm exact image catalog `google-gemini / gemini-3.1-flash-image / 1K`, version `google-gemini-image-2026-08-13`, USD 0.50/1M input và USD 60/1M image-output token; 1K documented output 1.120 token = 67.200 micro-USD trước input. Worker đọc actual model và Gemini `usageMetadata`, thu usage ngay khi response trả image bytes trước import/judge/selection, vì vậy candidate bị loại/không chọn/downstream failure và Gemini trước Pexels fallback vẫn được tính; Pexels chỉ tăng stock count và không bị gán model/giá Gemini. Per-run collector được truyền callback, không dùng mutable global state; completion/failure của Generation và Design Direction đều chuyển summary. Database aggregate homogeneous events, giữ heterogeneous là incomplete, snapshot text/image riêng và cộng known tokens/cost vào report; retry vẫn idempotent theo unique run. Migration 0019 chỉ thêm nullable image/provider/model/size/count/stock/token/pricing columns, không sửa migration 0018 và không DML backfill. Dashboard hiển thị text model, image model/1K/count, image token/source, Pexels count, total USD và breakdown text/image; historical row không được gán ngược image model. TDD RED hợp lệ: Usage Core thiếu image APIs/catalog; Worker bỏ `usageMetadata` và accounting event; Database từ chối `mediaUsage`/thiếu columns-report; Dashboard không tìm thấy image model/token/source/stock/breakdown. GREEN focused/package: Usage Core 14/14; Worker 9 files/95 tests; Database 12 files/96 tests; Web Usage API+UI 9/9 và full Web 48 files/399 tests; typecheck/lint của cả bốn target pass. Drizzle check sau khi nạp existing `.env` vào process báo `Everything's fine`; local PostgreSQL ban đầu dừng nên migration lượt đầu fail, sau khi chỉ start service PostgreSQL (không reset/xóa volume) thì 0019 apply thành công. Schema query xác nhận `image_model`, `image_total_estimated_micro_usd`, `stock_count`; 152 historical usage rows vẫn tồn tại và cả 152 giữ `image_model IS NULL`, chứng minh không backfill đoán. ADR-0021 và observability docs ghi provider-boundary accounting, Pexels distinction, immutable snapshot, partial/unpriced semantics và PostgreSQL authority. Final workspace gates: tests 35/35 Turbo tasks; coverage 35/35 tasks, Web đạt 85.21% statements, 80.03% branches, 82.55% functions, 89.05% lines; typecheck/lint/build đều 19/19 tasks. Production build chỉ có warning Windows PGlite link-name dài không gây fail. Bounded privacy scan không tìm thấy prompt, image bytes, object key, raw provider payload, credential hoặc LangSmith identifier trong Usage contract/API; final `git diff --check` sạch whitespace, chỉ có LF→CRLF notices. Không gọi Gemini/Pexels/LangSmith/Vercel, không sửa `.env`/local auth/GitHub OAuth/quota/admission/token budget, không commit/push và giữ `note.txt` ngoài scope.
- [x] Follow-up tách component Usage văn bản/hình ảnh ngày 2026-08-19: ảnh vỡ trong Editor được owner xác nhận chỉ do quên bật ngrok; sau khi bật tunnel ảnh hiển thị bình thường nên không sửa asset server, public route, Worker, Editor hoặc origin config. Usage report thêm explicit nullable `text` component từ đúng các cột text đã lưu, giữ `image` riêng và giữ aggregate token cấp item cho KPI/chart; `text: null` khi input/output/total text đều bằng 0, không dùng model đã claim hoặc priced-zero để suy đoán text usage. Combined pricing chỉ cộng/đánh giá text khi component thật sự tồn tại, nên image-only không còn bị text priced-zero ảnh hưởng; text-only, mixed, Pexels-only và historical partial/unpriced vẫn giữ semantics, không migration và không backfill. Dashboard chỉ render modality phát sinh: text và image có model/provider, input/output/total cùng cost riêng; image-only không còn `Văn bản: gemini-3.1-flash-lite` hay `$0.000000`, còn ba cột aggregate được ghi rõ `Tổng lượt`. TDD RED hợp lệ: Usage Core strict schema từ chối key `text`; Database có ba failure vì mapper thiếu text/null component; Web có ba failure vì thiếu token component và còn label cost cũ. GREEN focused: Usage Core 14/14; Database generation + usage 21/21; Web Usage Dashboard 6/6 với timeout test 30 giây (lượt mặc định 10 giây timeout do môi trường, không phải assertion failure). Full Web sau khi bổ sung `text` cho strict API fixture pass 48 files/400 tests; Database riêng pass 12 files/96 tests khi dùng timeout 30 giây. Coverage riêng: Usage Core 100/98.30/100/100; Database và Web full configured gates đều pass với mọi threshold >=80%. Workspace typecheck/lint/build pass 19/19 tasks; build chỉ có warning Windows PGlite link-name dài không gây fail. Workspace test mặc định và serial vẫn fail do nhiều PGlite integration hook chạm giới hạn 10 giây trong lúc chạy package Database, trong khi cùng 96 tests chạy riêng với timeout 30 giây đều xanh; đây là contention/timing hạ tầng đã tái hiện, không phải assertion Usage failure. `git diff --check` không có whitespace error, chỉ LF→CRLF notices. Observability docs đã đồng bộ component display và PostgreSQL authority. Build-generated `apps/web/next-env.d.ts` đã restore về trạng thái dev có chủ đích. Không sửa `.env`/auth/ngrok/asset flow, không gọi Gemini/Pexels/LangSmith/Vercel, không commit/push và giữ `note.txt` cùng file `-D` ngoài scope.
- [x] Follow-up ổn định CPU/RAM Next.js dev ngày 2026-08-19: tiến trình gây tải được xác định là Next.js 16.2.11 Turbopack dev server, không phải Worker/Gemini/LangSmith/ngrok. Trước fix, PID Next dùng khoảng 3.041–3.098 MiB Working Set, 2.549–2.612 MiB Private Memory và 26,6–30,2% CPU máy liên tục; log có compile 477,8 giây, filesystem cache compaction 47–67 giây và tự restart khi gần memory threshold. Generated `apps/web/.next/dev` đã phình 3.756,3 MiB, riêng `.next/dev/cache/turbopack` khoảng 3.504 MiB với nhiều SST 174–250 MiB. Đã dừng đúng cây ZenUI cũ, xác nhận các cổng 3000/3001/3002/9464 được giải phóng, không xóa dữ liệu/volume/`node_modules`/`.env`; sau đó chỉ xóa generated Turbopack cache. TDD RED hợp lệ: regression dev config mong `experimental.turbopackFileSystemCacheForDev === false` nhưng nhận `undefined`; GREEN cấu hình Next tắt đúng cache dev này và `pnpm dev:test` pass 10/10. Clean start vẫn dùng Turbopack, ready trong 751ms và log xác nhận experiment bị tắt; Web/Preview/Asset/Worker đều ready. Sáu mẫu idle đầu và sáu mẫu sau khi tải `/dashboard` + `/dashboard/usage` đều 0% CPU, không còn cache Turbopack trên disk; Next ổn định khoảng 499 MiB Private trước tải và 882 MiB sau compile hai route. Sau khi owner/browser tiếp tục mở nhiều Dashboard/project route và Web production build chạy kiểm chứng, Next giữ 0% CPU ở mẫu 10 giây, khoảng 1.298 MiB Private — cao hơn benchmark route tối thiểu nhưng không tăng CPU, không có compaction loop hoặc memory-threshold restart. Verification: `pnpm dev:test` 10/10, Web typecheck/lint pass, Web production build pass, `git diff --check` không có whitespace error ngoài LF→CRLF notices; build-generated `next-env.d.ts` đã restore về dev import. Không chuyển sang Webpack vì Turbopack sạch đã ổn định; không sửa `.env`, auth, asset/ngrok, provider behavior hoặc dữ liệu và chưa commit/push.
- [x] Follow-up giản lược bảng chi tiết AI Usage ngày 2026-08-19: theo phản hồi owner, mỗi component trong cột Model chỉ giữ dòng chính in đậm (`Văn bản: <model>` và/hoặc `Ảnh: <model> · <size> · <count>`); loại bỏ toàn bộ dòng phụ provider, token theo component, nguồn token, số ảnh Pexels và breakdown chi phí văn bản/ảnh. Ba cột Input/Output/Tổng token chỉ giữ số aggregate của lượt, không còn nhãn lặp `Tổng lượt`; cột chi phí chỉ giữ một tổng USD hoặc nhãn unpriced chính. Với record partial, dòng cảnh báo tổng hợp `role=alert` vẫn giữ để không làm người dùng hiểu nhầm subtotal là chi phí đầy đủ, nhưng lý do nhỏ lặp lại trong từng row đã bỏ. TDD RED trước production xác nhận các dòng phụ vẫn tồn tại; sau fix, focused Usage Dashboard pass 6/6, Web typecheck và lint pass, `git diff --check` không có whitespace error ngoài LF→CRLF notices. Runtime `/dashboard/usage` trên topology đang chạy trả HTTP 200 và Next hot-reload compile 21ms; không sửa report/API/database/pricing, asset/ngrok, `.env`, auth hoặc provider behavior; không gọi dịch vụ live và chưa commit/push.
- [x] Follow-up logo ZenUI trong Editor quay về trang chủ ngày 2026-08-19: root cause là chữ `ZenUI` trong toolbar chỉ là `<strong>` nên không có hành vi điều hướng. TDD RED hợp lệ chạy đúng regression mới và không tìm thấy accessible link `Về trang chủ ZenUI`; fix tối thiểu thay bằng Next.js `Link` nội bộ tới `/`, giữ nguyên vị trí/gradient qua class `.editor-toolbar-brand` và bổ sung focus-visible cho bàn phím. Không thêm handler JavaScript, dialog hay can thiệp autosave/document commands. GREEN focused 1/1 và full `editor.test.tsx` 39/39; Web typecheck/lint pass; `git diff --check` không có whitespace error ngoài LF→CRLF notices. Runtime trang chủ `http://localhost:3000/` trả HTTP 200 và chứa ZenUI; không sửa `.env`, auth, asset/ngrok, database hoặc provider behavior, không gọi dịch vụ live và chưa commit/push.
- [x] Follow-up đồng bộ gradient logo ZenUI toàn Web ngày 2026-08-19: audit xác nhận Landing header, Dashboard sidebar và chữ brand Login còn màu đen; Guided Onboarding dùng gradient tối riêng, trong khi Landing footer và Editor có treatment khác nhau. TDD RED hợp lệ chạy bốn test files và fail đúng 6 regression vì thiếu class chung trên Landing header/footer, Login, Dashboard, Guided Brief/Gallery và Editor. Fix thêm primitive `.zenui-brand-gradient` chỉ sở hữu gradient tím `#4f46e5 → #a855f7` + browser text clipping, giữ toàn bộ font-size/layout/link/focus ở selector từng surface; gắn class vào đúng logo/wordmark, không áp cho heading/câu văn chứa “ZenUI”, không thay local login/GitHub khóa hoặc flow navigation/onboarding/editor. GREEN 4 files/68 tests; Web typecheck/lint pass; `git diff --check` không có whitespace error ngoài LF→CRLF notices. Runtime Chrome xác nhận Landing header/footer, Login và Dashboard đều có computed gradient/text clip; heading Landing vẫn màu slate bình thường; mobile 390px logo nằm trong viewport; Dashboard logo focus + Enter quay về `/`. Web/Preview/Asset/Worker tiếp tục listen ở 3000/3001/3002/9464; Landing/Login HTTP 200. Không gọi provider live, không sửa `.env`, auth, asset/ngrok, database/quota và chưa commit/push.
- [x] Follow-up hiển thị chi phí AI bằng Việt Nam đồng ngày 2026-08-19: chỉ lớp trình bày Dashboard quy đổi integer micro-USD theo tỷ giá cố định được công khai `1 USD = 26.000₫`; database, immutable pricing snapshots, Usage API/schema và `currency: USD` giữ nguyên làm nguồn kế toán. Formatter dùng `BigInt` và half-up đến đồng gần nhất, không dùng floating point hoặc API tỷ giá; ví dụ 67.400 micro-USD → `1.752₫`, 97.459 → `2.534₫`, 134.550 → `3.498₫`, known zero → `0₫`. KPI, record `priced` và known subtotal `partial` hiển thị VND; cảnh báo tổng chưa đầy đủ vẫn giữ, còn `unpriced` tiếp tục hiện nguyên nhân và không bị biến thành `0₫`. TDD RED hợp lệ: focused Usage Dashboard chạy 6 tests, 3 fail đúng vì còn USD/thiếu VND và ghi chú tỷ giá; GREEN sau implementation và regression unpriced đạt 7/7. Web typecheck/lint pass; `git diff --check` không có whitespace error ngoài LF→CRLF notices. Runtime Chrome desktop 1440×1000 và mobile 390×844 đều hiện KPI `7.240₫`, ghi chú tỷ giá, các row VND, không còn USD hiển thị và không có page-level horizontal overflow; bảng mobile vẫn dùng local horizontal scroll có chủ đích. Web/Preview/Asset/Worker tiếp tục listen ở 3000/3001/3002/9464. Không gọi Gemini/Image/Pexels/LangSmith/Vercel, không sửa `.env`, auth, database/migration, pricing catalog, quota, generation hay asset/ngrok; không backfill dữ liệu lịch sử và chưa commit/push.
- [x] Stable checkpoint trước push ngày 2026-08-19: full workspace coverage chạy tuần tự ban đầu đạt 34/35 task nhưng Web branch coverage còn 79,98%, thiếu ngưỡng 80% đúng 0,02 điểm. Bổ sung regression cho ba nhãn `unpriced` còn thiếu (`missing_image_input_usage`, `unknown_image_model`, `heterogeneous_image_usage`) mà không đổi production behavior; full Web đạt 48 files/405 tests và coverage 85,25% statements, 80,06% branches, 82,55% functions, 89,09% lines. Full workspace coverage rerun pass 35/35 tasks; typecheck/lint/build đều pass 19/19 tasks. Production build chỉ có warning Windows non-fatal về PGlite link name dài; build-generated `apps/web/next-env.d.ts` đã restore về dev import có chủ đích. `git diff --check` không có whitespace error ngoài LF→CRLF notices. Secret scan chỉ phát hiện credential trong `.env` local bị Git ignore và các giá trị test/ghi chú an toàn; `.env`, `note.txt` và file `-D` được giữ ngoài commit. Không gọi provider live.
- [x] Follow-up điều hướng từ Câu chuyện trang tới Canvas ngày 2026-08-19: desktop và sheet màn hình hẹp dùng cùng một Page Story handler nhưng vẫn giữ `selectEditorNode` làm nguồn selection; chỉ thao tác từ Câu chuyện trang queue viewport navigation, còn Canvas/Layers/Preview/Inspector và command/history/autosave không đổi. Desktop phát hiện `.editor-center-scroll` là scroll owner thật qua computed overflow, tính offset section theo geometry và gọi `scrollTo`, nên sidebar và browser body không bị kéo theo; mobile nơi center `overflow: visible` dùng native `scrollIntoView`, sheet đóng sau khi chọn. `prefers-reduced-motion` đổi smooth thành auto; target lookup bị giới hạn trong center và so exact `data-node-id`. TDD RED ban đầu 2/37 fail vì chưa có navigation; regression scroll-ownership siết lại tiếp tục RED 2/38 vì implementation còn dùng `scrollIntoView` trên desktop. GREEN cuối focused Simple Editor 38/38; Web typecheck/lint pass; `git diff --check` không có whitespace error ngoài LF→CRLF notice. Runtime Chromium trên project nhiều section xác nhận desktop center `scrollTop 0→5332`, Page Story `0→0`, body `0→0`, Footer selected/visible và không overflow ngang; mobile 390×844 đóng sheet, Footer selected/visible và không overflow ngang. Không sửa CSS, Design Document, command/history/autosave, AI/API/database/auth/assets/ngrok hoặc `.env`; không gọi provider live, không commit/push.
